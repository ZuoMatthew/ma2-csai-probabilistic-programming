% Note that all variables are taken into account when grounding an AD, that is, in the following example, the probability of a ball being red increases with the number of games it was used for:
